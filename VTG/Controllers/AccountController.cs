...
namespace VTG.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        [AllowAnonymous]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> Register(RegisterViewModel model, /*this is image of tourist*/HttpPostedFileBase profileImage)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    {
                        fileExtension = Path.GetExtension(profileImage.FileName);
                        fileName = user.Id + fileExtension;
                        string path = Path.Combine(Server.MapPath("~/profile images"), fileName);
                        profileImage.SaveAs(path);
                    }
                    user.ProfileImage = "~/profile images/" + fileName;
                    ApplicationDbContext dbIdentity = new ApplicationDbContext();
                    var roleManager = new RoleManager<IdentityRole>(new RoleStore<IdentityRole>(dbIdentity));
                    if (roleManager.RoleExists("Tourists"))
                    {
                        UserManager.AddToRole(user.Id, "Tourists");
                    }
        ...
        public async Task<ActionResult> ForgotPassword(ForgotPasswordViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                string code = await UserManager.GeneratePasswordResetTokenAsync(user.Id);
                var callbackUrl = Url.Action("ResetPassword", "Account", new { userId = user.Id, code = code }, protocol: Request.Url.Scheme);
                mail.From = new MailAddress("m.refaee123580@gmail.com");
